---
title: 发布说明
description: "Zod 4 发布说明和新功能，包括性能改进和重大变更"
---
...
```ts
emailSchema.register(myRegistry, { title: "电子邮箱地址", description: "..." })
// => 返回 emailSchema
```

### 全局注册表

Zod 还导出一个全局注册表 `z.globalRegistry`，支持一些常见的、与 JSON Schema 兼容的元数据：
...
为了方便将模式添加到 `z.globalRegistry`，可以使用 `.meta()` 方法。
...
请参阅[自定义错误](/error-customization#locales)中的完整支持语言列表；该部分会在有新支持语言时及时更新。
...
详见 [`z.stringbool()` 文档](/api#stringbool)。
