...
在文档、代码生成、AI 结构化输出、表单验证及其他用途中，为某些架构关联一些额外的*元数据*常常非常有用。

## 注册表

Zod 中的元数据通过*注册表*进行管理。注册表是架构的集合，每个架构都与某些*强类型*元数据关联。要创建一个简单的注册表：
...
要全局扩展 `GlobalMeta` 接口，使用[*声明合并*](https://www.typescriptlang.org/docs/handbook/declaration-merging.html)。在代码库中的任意位置添加以下内容。通常的约定是在项目根目录创建一个 `zod.d.ts` 文件。
