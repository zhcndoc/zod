...
你可以将 `unrepresentable` 选项设置为 `"any"` 来改变此行为。这会将任何不可表示的类型转换为 `{}`（JSON Schema 中 `unknown` 的等价形式），并从包含该类型的 schema 中移除不可表示的校验。
...
无法序列化为 JSON 的默认 *值* 会像 `.default()` schema 一样经过相同的处理函数。在 `"any"` 模式下，默认值会被丢弃，其余 schema 会照常生成；返回一个 JSON Schema 即可提供你自己的表示。
...
为字面量返回一个 JSON Schema 会替换整个字面量，并丢弃其中可表示的成员：`z.literal(["a", 1n])` 会变成你返回的内容，而 `"a"` 会被移除。返回 `"any"` 则会保留现有的按值处理行为。
...
`z.iso.datetime()` 的 `local: true` 和 `precision: -1` 变体会回退到 `pattern`，因为 RFC 3339 `date-time` 要求同时包含偏移量和秒数。

以下 schema 通过 `contentEncoding` 支持：
...
同样，`nullable` 会将 `"null"` 添加到可接受类型集合中。无法写成裸类型的内部 schema 会回退为 `anyOf`：
