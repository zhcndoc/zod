...
在其他情况下，你可能有一组相互关联的 Zod schemas，想用多个相互引用的 JSON Schema 表示，或写入 `.json` 文件并用做 Web 服务。这时你可以传入一个[注册表](/metadata#registries)给 `z.toJSONSchema()`。
...
上面两个 schema 都在 `z.globalRegistry` 中注册了 `id` 属性。要将它们转换成 JSON Schema，将整个注册表传给 `z.toJSONSchema()`。
...
> 注册表中任何带 `id` 属性的 schema 都将被提取到 `schemas` 中。

默认情况下，`$ref` URI 是相对路径，如 `"User"`。要将其改为绝对 URI，使用 `uri` 选项。该选项接收一个将 `id` 转换为完整 URI 的函数。
