...
使用 `z.invertCodec()` 从现有编解码器派生反向编解码器。返回的编解码器会交换输入和输出模式，然后交换 `decode` 和 `encode` 转换。
...
`z.invertCodec()` 只会反转你传给它的那个编解码器。它不会递归反转嵌套在另一个模式中的编解码器；请在你定义反向模式的地方反转这些编解码器。
...
> **有趣的事实** — Codecs 在内部实现为带有“中间”转换逻辑的 pipe 子类。 
