...
> `zod/v4` 包使用了该类的子类，称为 `z.ZodError`，它实现了一些额外的便捷方法。
...
返回 `undefined` 可避免设置错误消息。这对于只定制特定错误消息而忽略其他情况很有用。Zod 会在[优先级链](#错误优先级)中交出控制权给下一个错误映射。
...
要指定全局错误映射，可使用 `z.config()` 设置 Zod 的 `customError` 配置：
...
> **注意** — `zod/v4` 库自动加载 `en` 语言包。`zod/v4-mini` 子包不加载任何语言包，所有错误信息默认都是 `Invalid input`。
...
为了方便，所有语言包作为 `z.locales` 从 `zod/v4` 导出。在某些打包器中，这可能无法进行摇树优化。
