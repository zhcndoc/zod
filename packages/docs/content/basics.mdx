...
<Callout>
**注意** — 如果你的 schema 使用了某些异步 API，例如 `async` [细化](#refine) 或 [转换](#transform)，你需要使用 `.parseAsync()` 方法。
...
<Callout>
**注意** — 如果你的 schema 使用了某些异步 API，例如 `async` [细化](#refine) 或 [转换](#transform)，你需要使用 `.safeParseAsync()` 方法。
