...
该模式将产生 `"invalid_format"` 问题，这比通过细化或 `z.custom()` 产生的 `"custom"` 错误更具描述性。
...
`.safeExtend()` 方法的工作方式类似于 `.extend()`，但它不会让你用不可分配的模式覆盖现有的属性。换句话说，`.safeExtend()` 的结果将具有一个推断类型，该类型 [`extends`](https://www.typescriptlang.org/docs/handbook/2/conditional-types.html#conditional-type-constraints) 原始类型（在 TypeScript 的意义上）。
