...
<Callout>
**替代方案：解构** — 你可以完全避免使用 `.extend()`，通过创建一个全新的对象模式。这使得结果模式的严格程度在视觉上变得明显。
...
1. 它使用语言级特性（[解构语法](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring)），而不是特定于库的 API
2. 相同的语法在 Zod 和 Zod Mini 中都适用
3. 它在 `tsc` 上更高效 — `.extend()` 方法在大型模式上可能很昂贵，并且由于 [TypeScript 的限制](https://github.com/microsoft/TypeScript/pull/61505)，当调用链式调用时，它的成本会呈平方增长
4. 如果需要，您可以通过使用 `z.strictObject()` 或 `z.looseObject()` 来更改结果模式的严格性级别
</Callout>
