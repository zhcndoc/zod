...
datetime.parse("2020-01-01T00:00:00.123456Z"); // ✅（任意精度）
datetime.parse("2020-01-01T00:00:00+02:00"); // ❌（不允许偏移）
```
...
datetime.parse("2020-01-01T00:00:00.123+02:00"); // ✅（毫秒可选）
datetime.parse("2020-01-01T00:00:00.123+0200"); // ✅（毫秒可选）
datetime.parse("2020-01-01T00:00:00.123+02"); // ✅（仅偏移小时）
datetime.parse("2020-01-01T00:00:00Z"); // ✅（Z 仍支持）
```
...
`z.iso.time()` 方法验证格式为 `HH:MM:SS[.s+]` 的字符串。秒数允许任意小数精度。不允许任何时区偏移。
...
time.parse("00:00:00"); // ✅
time.parse("09:52:31"); // ✅
time.parse("23:59:59.9999999"); // ✅（任意精度）

time.parse("00:00:00.123Z"); // ❌（不允许 `Z`）
time.parse("00:00:00.123+02:00"); // ❌（不允许偏移）
```

可以通过 `precision` 参数限制允许的小数位数。
...
<Callout>
  **Zod 4** — 在 Zod 4 中，若将 `z.enum` 传给 `z.record()` 作为第一个参数，Zod 会详尽检查输入对象是否包含所有枚举值作为键。此行为与 TypeScript 保持一致：
  ...
  Zod 3 中未检查详尽。若想复现 Zod 3 表现，可用 `z.partialRecord()`。
