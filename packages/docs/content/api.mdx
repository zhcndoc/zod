...
**New** — 自 v4.2 起，Zod 已能正确支持 Record 中的数值键，其行为更接近 TypeScript 本身。`number` 模式在用作 Record 键时，会验证该键是否为有效的“数字字符串”。其他数值约束（min、max、step 等）也会一并验证。
...
fileSchema.check(
  ...
  z.mime(["image/png", "image/jpeg"]); // 多种 MIME 类型
)
```
</Tab>
</Tabs>
...
> **新** — 在 Zod 4.1 中引入。有关更多信息，请参阅专门的 [编解码器](/codecs) 页面。
...
请参阅专门的 [编解码器](/codecs) 页面以获取更多信息。该页面包含您可以复制/粘贴到项目中的常用编解码器的实现：
