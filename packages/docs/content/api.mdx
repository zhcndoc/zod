...
允许缺少键，但不允许显式传入 `undefined`，符合 TypeScript 的[`exactOptionalPropertyTypes`](https://www.typescriptlang.org/tsconfig/#exactOptionalPropertyTypes)：
...
与 `.partial()` 相同，但它会将每个字段包装在 [`exactOptional()`](#exact-optionals) 中，而不是 `optional()`：
...
只有 codecs 具有两个真正的端。对单向 [transform](#transforms) 使用 `z.output()` 时，会返回该 transform，而它不会验证任何内容。
